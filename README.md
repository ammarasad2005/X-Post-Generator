...
2. **Configure environment variables.** Copy the example file and fill in your keys:
   ...
   | `OPENROUTER_PRIMARY_MODEL` | No | Override the primary free model. Default: `nvidia/nemotron-3-ultra-550b-a55b:free` (most reliable as of 2026-07-07). Browse all free models at https://openrouter.ai/models?q=free |
...
- **Backend:** One API route (`app/api/generate/route.ts`) that:
  ...
  3. Calls OpenRouter's free-tier models — tries the primary model first (`OPENROUTER_PRIMARY_MODEL`, default `nvidia/nemotron-3-ultra-550b-a55b:free`), falls back through `meta-llama/llama-3.3-70b-instruct:free` → `openai/gpt-oss-120b:free` → `qwen/qwen3-coder:free` if the primary returns an error or unparseable JSON.
...
- **Note on free model availability:** OpenRouter periodically moves models between free and paid tiers AND applies global rate limits to popular free models during peak hours. As of 2026-07-07: `google/gemini-2.5-flash:free` is paid-only, and `meta-llama/llama-3.3-70b-instruct:free` / `openai/gpt-oss-120b:free` / `qwen/qwen3-coder:free` are frequently globally rate-limited. The app defaults to `nvidia/nemotron-3-ultra-550b-a55b:free` (1M context, 550B params, lower traffic) which has been the most reliable. Browse the current free models at https://openrouter.ai/models?q=free and override via `OPENROUTER_PRIMARY_MODEL` if needed.
...
| Generate requests return `UPSTREAM_INVALID_RESPONSE` (HTTP 502) | All free models returned unparseable JSON | Try a different `OPENROUTER_PRIMARY_MODEL` (some free models struggle with strict JSON schema). Recommended reliable choices: `nvidia/nemotron-3-ultra-550b-a55b:free`, `meta-llama/llama-3.3-70b-instruct:free`, or `qwen/qwen3-coder:free` |
